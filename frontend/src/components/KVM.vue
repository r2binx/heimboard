	...
	<n-table :striped="true">
		<n-tbody>
			<n-tr v-for="vm in state.vms" :key="vm.name">
				<n-td>
					<n-collapse v-if="vm.state !== 'shutoff'" @item-header-click="expandHandler">
						<n-collapse-item
							:key="vm.name"
							:title="vm.name.toUpperCase()"
							:name="vm.name"
						>
							<n-space
								v-if="vm.mem_modifiable"
				...
				<n-td>
					...
					<template v-else>
						<n-space :vertical="windowWidth < 900" style="float: right">
							<n-popconfirm
								v-if="expandedItems[vm.name] && vm.state !== 'paused'"
							...
							<n-popconfirm
								v-if="expandedItems[vm.name]"
								@positive-click="confirmDestroy(vm.name)"
							>
								<template #trigger>
									<n-button
										tertiary
